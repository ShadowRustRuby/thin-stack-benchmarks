...
function filterClientSide(query) {
  ...
  const filtered = cachedRecords.filter((r) => r.title.toLowerCase().includes(normalizedQuery) || r.artist.toLowerCase().includes(normalizedQuery) || r.genre.toLowerCase().includes(normalizedQuery) || r.year.toLowerCase().includes(normalizedQuery) || r.liked_by.toLowerCase().includes(normalizedQuery));
  ...
  targetElement.innerHTML = records.map((r) => `
        <div class="record-card">
            ...
            <div class="record-meta">
                ...
                ${r.liked_by ? `<div class="like-badge" style="margin-top: 0.4rem;">❤️ Liked by: ${escapeHtml(r.liked_by)}</div>` : ""}
            </div>
        </div>
    `).join("");
}
