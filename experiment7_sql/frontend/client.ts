...
async function initialize() {
    try {
        ...
        // Fetch baseline relational records from Assembly SQLite API (Port 8081)
...
function filterClientSide(query: string) {
    ...
    const filtered = cachedRecords.filter(r => 
        ...
        r.liked_by.toLowerCase().includes(normalizedQuery)
    );
...
// Fetch from SQLite backend API at specific port
