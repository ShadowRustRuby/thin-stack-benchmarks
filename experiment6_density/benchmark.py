...
def run_bash(cmd):
    # Runs command with newgrp docker to ensure we have daemon permissions
...
def get_image_sizes():
    ...
    for line in stdout.split('\n'):
        if ':' in line:
            ...
            if 'experiment6_density' in repo:
                sizes[repo] = size
    return sizes
...
def get_container_stats():
    ...
    for line in stdout.split('\n'):
        ...
        if len(parts) == 3:
            name, cpu, mem = parts
            # Clean name (remove container prefix/suffix if present)
            name = name.strip()
            stats[name] = {
                'cpu': cpu.strip(),
...
def run_load_test(port, num_requests=500, concurrency=10):
    ...
    # Track statistics during load test
            ...
            if c_name in stats:
                try:
                    ...
                    if cpu_val > peaks['cpu']:
                        peaks['cpu'] = cpu_val
                    peaks['mem'] = stats[c_name]['mem'] # Just keep last/highest reading
                except Exception:
                    pass
            time.sleep(0.1)
...
def main():
    print("=== Executing Thin Stack Benchmark Suite ===")
    ...
    print("\n[3/3] Load Testing Endpoints (500 requests @ 10 concurrency)...")
    ...
    for label, port, container_name, img_suffix in targets:
        print(f"  Benchmarking {label} API on port {port}...")
