...
// Helper to decode URL percent encoding
void url_decode_c(char *dst, const char *src) {
    char a, b;
    while (*src) {
        if ((*src == '%') &&
            ((a = src[1]) && (b = src[2])) &&
...
int match_record_c(const char *text, const char *query) {
    ...
    url_decode_c(decoded_query, query);
    if (decoded_query[0] == '\0') return 1;
    ...
    strncpy(query_lower, decoded_query, 255);
    query_lower[255] = '\0';
