...
// Link to our shared parser and SSL helper library functions
struct SSL_CTX;
struct SSL;
...
int main() {
    ...
    SSL_CTX *ssl_ctx = ssl_init_c();
    if (!ssl_ctx) {
        fprintf(stderr, "Error: Failed to initialize SSL context (ensure server.crt and server.key exist)\n");
        return 1;
    }
    ...
    printf("Secure (HTTPS) C++ API Server running on port %d...\n", PORT);
    ...
    while (1) {
        ...
        SSL *ssl = ssl_accept_c(ssl_ctx, client_fd);
        if (!ssl) {
            close(client_fd);
            continue;
        }
        ...
        ssl_read_c(ssl, request_buffer, BUFFER_SIZE - 1);
        ...
        // Send HTTP Response Headers and Body via SSL
        ...
        ssl_write_c(ssl, response_header, strlen(response_header));
        ssl_write_c(ssl, json_body, content_length);
