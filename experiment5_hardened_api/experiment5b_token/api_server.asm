...
.section .bss
    ...
    .lcomm expected_signature, 65 # 64-char hex signature + null terminator
    .lcomm sig_header_str, 128    # formatted header query X-Signature: <hex>
...
accept_loop:
    ...
    # Read request: read(client_fd, client_buffer, 2048)
    movq $0, %rax                # sys_read
    movq %r13, %rdi
...
do_signature_check:
    ...
    # hmac_sha256_hex(query_string, hmac_key, expected_signature)
    movq $query_string, %rdi     # 1st arg: msg
    movq $hmac_key, %rsi         # 2nd arg: key
    movq $expected_signature, %rdx # 3rd arg: out_hex
    subq $8, %rsp                # 16-byte stack align
    call hmac_sha256_hex
    addq $8, %rsp

    # Format check header "X-Signature: <expected_signature>" in sig_header_str
    movq $sig_header_str, %rdi
    movq $x_signature_header_prefix, %rsi
    call strcpy_asm
    movq %rdi, %r10              # r10 points to end of prefix (null char)
    movq $expected_signature, %rsi
    movq %r10, %rdi
    call strcpy_asm

    # Scan client_buffer for sig_header_str
    movq $client_buffer, %rdi
    movq $sig_header_str, %rsi
    call strstr_asm
    cmpq $0, %rax
    jne auth_success
