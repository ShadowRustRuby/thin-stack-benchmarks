...
int main() {
    ...
    while (1) {
        ...
        strcpy(req_copy, request_buffer);

        char *req_line = strtok(req_copy, "\r\n");
        char query_str[128] = "";
        ...
        // Calculate expected HMAC signature of the query string using key "secret"
        char expected_sig[65];
        hmac_sha256_hex(query_str, "secret", expected_sig);

        // Format signature header search string: "X-Signature: <sig>"
        char sig_header_str[128];
        snprintf(sig_header_str, sizeof(sig_header_str), "X-Signature: %s", expected_sig);

        // Security check: Verify X-Signature matches the expected signature
        if (!strstr(request_buffer, sig_header_str)) {
